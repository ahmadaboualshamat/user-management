    ...
    stages {
        stage('GIT Checkout') {
            steps {
                ...
                checkout scmGit(branches: [[name: '*/release/qa']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/ahmadaboualshamat/user-management']])
            }
        }
        stage('Build Maven') {
            steps{   
        ...
        stage('Build Image') {
            steps{ 
                script{
                    bat 'docker build -t ahmadaboualshamat/user-management .'
                }
            }
        }
